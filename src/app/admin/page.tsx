...
import { CATEGORY_LABELS, ORDER_STATUS_LABEL, ORDER_STATUS_VARIANT, formatDate, isAdmin } from '@/lib/utils'
...
export default function AdminPage() {
  ...
  useEffect(() => {
    const init = async () => {
      ...
      // Fetch all data
      const [orders, resources, members, referrals, products] = await Promise.all([
        fetch('/api/admin/data?table=orders', { headers }).then(r => r.json()),
        fetch('/api/admin/data?table=resources', { headers }).then(r => r.json()),
        fetch('/api/admin/data?table=members', { headers }).then(r => r.json()),
        fetch('/api/admin/data?table=referrals', { headers }).then(r => r.json()),
        fetch('/api/admin/data?table=products', { headers }).then(r => r.json()),
      ])

      setData({ orders, resources, members, referrals, products })
      setLoading(false)
    }
    init()
  }, [])
            ...
            <tbody>
              {orders.map((o: any) => (
                <tr key={o.id} className="border-b border-qblue/5 hover:bg-qpanel/50">
                  ...
                  <td className="px-4 py-3"><Badge variant={ORDER_STATUS_VARIANT[o.status as keyof typeof ORDER_STATUS_VARIANT] ?? 'gray'}>{ORDER_STATUS_LABEL[o.status as keyof typeof ORDER_STATUS_LABEL] ?? o.status}</Badge></td>
