...
export function AdminOrderActions({ orderId }: { orderId: string }) {
  ...
  const call = async (action: 'approve' | 'reject') => {
    if (action === 'reject' && !rejectNote.trim()) { setShowRejectInput(true); return }
    ...
    try {
      ...
      setTimeout(() => { router.refresh(); window.location.reload() }, 1000)
    } catch (e: any) {
      setMsg('错误：' + e.message)
    } finally {
      setLoading(null)
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <button onClick={() => showRejectInput ? call('reject') : setShowRejectInput(true)} disabled={!!loading}
